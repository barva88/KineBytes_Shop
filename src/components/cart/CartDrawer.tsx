      ...
      <div className="absolute inset-y-0 right-0 w-full max-w-md bg-kb-surface border-l border-zinc-800 shadow-drawer animate-slide-in-right flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500/10 text-xs font-semibold text-emerald-400 border border-emerald-500/20">
              {itemCount}
            </span>
          </div>
          <button
          ...
          {cartProducts.length === 0 ? (
            ...
            cartProducts.map(({ item, product, variantLabel, unitPrice, lineTotal }) => (
              <div
                ...
                className="flex gap-4 p-4 rounded-xl border border-zinc-800/60 bg-zinc-900/40"
              >
                {/* Product icon */}
                ...
                <div className="flex-1 min-w-0 space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      {item.quantity > 1 && (
                        <p className="text-xs text-zinc-600">{formatCurrency(unitPrice)} c/u</p>
                      )}
                    </div>
                  </div>
                </div>
